...
def decide(request: Request, pid: str, cid: str, decision: str = Form(...), notes: str = Form("")):
    ...
    if d == Decision.YES:
        ...
        p.add_expansion([
            ExpansionCandidate(
                candidate_id=f"exp-{cid}",
                title=f"Related to: {c.title}",
                source=cid
            )
        ])
...
# ✅ simple auto collect placeholder
