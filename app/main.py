...
def _page_context(project_id: Optional[str] = None, selected_candidate_id: Optional[str] = None) -> dict[str, object]:
    ...
    graph_svg = render_graph_svg(project.graph if project else Graph(), selected_candidate_id=selected_candidate_id)
    return {
        'request': None,
...
def project_workspace(request: Request, project_id: str, candidate_id: Optional[str] = None) -> HTMLResponse:
    return _workspace_response(request, _page_context(project_id, candidate_id))


@app.post('/projects', response_class=HTMLResponse)
def create_project(request: Request, title: str = Form(...), description: str = Form('')) -> HTMLResponse:
...
def search_project(request: Request, project_id: str, query: str = Form(...), limit: int = Form(8)) -> HTMLResponse:
...
@app.post('/projects/{project_id}/candidates/{candidate_id}/select', response_class=HTMLResponse)
def select_candidate(request: Request, project_id: str, candidate_id: str) -> HTMLResponse:
    return _workspace_response(request, _page_context(project_id, candidate_id))


@app.post('/projects/{project_id}/candidates/{candidate_id}/decision', response_class=HTMLResponse)
def decide_candidate(request: Request, project_id: str, candidate_id: str, decision: str = Form(...), notes: str = Form('')) -> HTMLResponse:
    project = store.get(project_id)
    try:
...
def scan_library(request: Request, project_id: str, candidate_id: Optional[str] = Form(None)) -> HTMLResponse:
    project = store.get(project_id)
...
def export_project(request: Request, project_id: str, candidate_id: Optional[str] = Form(None)) -> HTMLResponse:
    project = store.get(project_id)
